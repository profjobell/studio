...
export default function SettingsPage() {
  ...
  const [addUserFormState, addUserFormAction, isAddingUserPending] =
    useReactActionState(addUserProfileAction, initialAddUserState);
  const [isAddUserDialogOpen, setIsAddUserDialogOpen] = useState(false);
  const addUserFormRef = useRef<HTMLFormElement>(null);

  const [conceptuallyAddedUsers, setConceptuallyAddedUsers] = useState<
    ConceptuallyAddedUserProfile[]
  >([]);
  ...
  const [userToDelete, setUserToDelete] =
    useState<ConceptuallyAddedUserProfile | null>(null);
  ...
  // Calculate admin status after client render
  useEffect(() => {
    ...
    if (bypassIsActive) {
      ...
      activeUserEmail &&
      ADMIN_EMAILS.includes(activeUserEmail.toLowerCase())
    ) {
      determinedIsAdminOnRender = true;
    }
  ...
  useEffect(() => {
    if (
      addUserFormState.message &&
      (addUserFormState.success || addUserFormState.errors)
    ) {
      if (addUserFormState.success) {
        toast({
          title: "User Action",
          description: addUserFormState.message,
        });
        ...
        toast({
          ...
          description: addUserFormState.message + formatZodErrors(addUserFormState.errors),
          variant: "destructive",
          duration: 7000,
        });
      }
  ...
  const handleFormSubmit = async (
    ...
    saveAction: (
      formData: FormData
    ) => Promise<{ success: boolean; message: string; errors?: ZodIssue[] }>,
    ...
    startTransition(async () => {
      ...
      if (result.success) {
        ...
        toast({
          title: "Save Failed",
          description: result.message + formatZodErrors(result.errors),
          variant: "destructive",
          duration: 7000,
        });
      }
  ...
  const handleDeleteConceptualUser = async () => {
    ...
    startDeletingUserTransition(async () => {
      ...
      if (result.success) {
        ...
        setConceptuallyAddedUsers((prevUsers) =>
  ...
  if (!authCheckCompleted && !isClientRender) {
    return (
  ...
  if (authCheckCompleted && !isUserAdmin) {
    return (
  ...
  if (isLoadingInitialData && isUserAdmin) {
    return (
      <div className="flex items-center justify-center min-h-[60vh]">
        <Loader2 className="h-12 w-12 animate-spin text-primary" />
  ...
  if (!settings && isUserAdmin && !isLoadingInitialData) {
    return (
    ...
    <>
      <div className="container mx-auto py-8 px-4 md:px-6 space-y-8">
          ...
          <CardContent>
            ...
            >
              ...
              <div className="space-y-2">
                ...
                <Select
                  name="defaultTheme"
                  defaultValue={settings.general.defaultTheme}
                >
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Manage AI model settings and safety filters.
            </CardDescription>
          </CardHeader>
          <CardContent>
            <form
            ...
            >
              ...
              <div className="space-y-1">
                ...
                <p className="text-xs text-muted-foreground">
                  Configure thresholds for content blocking.
                </p>
              </div>
              {[
                {
                  ...
                  value: settings.ai.safetyFilters.hateSpeech,
                },
                {
                  id: "safetyDangerousContent",
                  label: "Dangerous Content",
                  category: "HARM_CATEGORY_DANGEROUS_CONTENT",
                  value: settings.ai.safetyFilters.dangerousContent,
                },
                {
                  id: "safetyHarassment",
                  label: "Harassment",
                  category: "HARM_CATEGORY_HARASSMENT",
                  value: settings.ai.safetyFilters.harassment,
                },
                {
                  id: "safetySexuallyExplicit",
                  label: "Sexually Explicit",
                  category: "HARM_CATEGORY_SEXUALLY_EXPLICIT",
                  value: settings.ai.safetyFilters.sexuallyExplicit,
                },
              ].map((filter) => (
              ...
              <div className="space-y-2">
                ...
                <Input
                  type="password"
                  placeholder="Enter Google AI API Key (Handled via .env)"
                  disabled
                />
                <p className="text-xs text-muted-foreground">
                  API keys should be managed via environment variables for
                  security.
                </p>
              </div>
        ...
        <Card>
          ...
          <CardContent>
            ...
            >
              {[
                {
                  id: "podcastGeneration",
                  label: "Podcast Generation (Teaching Reports)",
                  checked: settings.featureFlags.podcastGeneration,
                },
                {
                  id: "personalizedFallacyQuiz",
                  label: "Personalized Fallacy Quizzes (from Reports)",
                  checked: settings.featureFlags.personalizedFallacyQuiz,
                },
                {
                  id: "scriptureMemoryTool",
                  label: "Scripture Memory Tool",
                  checked: settings.featureFlags.scriptureMemoryTool,
                },
                {
                  id: "ismAwarenessQuiz",
                  label: "Ism Awareness Quiz (Learning Tools)",
                  checked: settings.featureFlags.ismAwarenessQuiz,
                },
              ].map((feature) => (
                <div
                  key={feature.id}
                ...
                >
                  <Label htmlFor={feature.id} className="flex-grow">
                    {feature.label}
                  </Label>
                  <Switch
                    id={feature.id}
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Manage application data and users.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-6">
            <div>
              <h3 className="text-lg font-medium mb-2">
                Conceptually Added Users (Current Session)
              </h3>
              {isLoadingUsers && (
              ...
              {!isLoadingUsers && conceptuallyAddedUsers.length === 0 && (
                <p className="text-sm text-muted-foreground">
                  No conceptual users added yet in this session.
                </p>
              )}
              {!isLoadingUsers && conceptuallyAddedUsers.length > 0 && (
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              ...
              <FeaturesGuideModal>
                <Button variant="outline">
                  <Edit3 className="mr-2 h-4 w-4" /> View/Edit 'Learn More'
                  Guide
                </Button>
              </FeaturesGuideModal>
                    ...
                    >
                      ...
                      {addUserFormState.errors && (
                        <ul className="text-sm text-destructive list-disc list-inside">
                          {addUserFormState.errors.map((err, i) => (
                            <li key={i}>
          ...
          <DeleteAlertDialogContent>
            <DeleteAlertDialogHeader>
              ...
              <DeleteAlertDialogDescription>
                This will remove the conceptual user profile for &quot;
                {userToDelete.newDisplayName}&quot; (
                {userToDelete.newUserEmail}) from the current session.
              </DeleteAlertDialogDescription>
            </DeleteAlertDialogHeader>
      ...
      <FeaturesGuideModal>
        <span className="hidden">
          Hidden Trigger for Programmatic Features Guide Modal
        </span>
      </FeaturesGuideModal>
    </>
  );
